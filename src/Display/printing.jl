...
function Base.show(io::IO, dftu::DFTU)
    print(io, "l=$(dftu.l), types=$(dftu.types), manifolds=$(dftu.manifolds), values=$(dftu.values) eV, α=$(dftu.α), β=$(dftu.β), projection=$(dftu.projection_type)")
end
